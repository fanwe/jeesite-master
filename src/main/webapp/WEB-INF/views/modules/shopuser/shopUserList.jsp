...
<body>
	...
	<form:form id="searchForm" modelAttribute="shopUser" action="${ctx}/shopuser/shopUser/" method="post" class="breadcrumb form-search">
		...
		<c:forEach items="${page.list}" var="shopUser">
			<tr>
				...
				<shiro:hasPermission name="shopuser:shopUser:edit"><td>
    				<a href="${ctx}/shopuser/shopUser/form?id=${shopUser.id}">修改</a>
					<a href="${ctx}/shopuser/shopUser/islock?id=${shopUser.id}" onclick="return confirmx('确认要冻结该会员吗？', this.href)">冻结</a>
					<a href="${ctx}/shopuser/shopUser/delete?id=${shopUser.id}" onclick="return confirmx('确认要删除该会员吗？', this.href)">删除</a>
